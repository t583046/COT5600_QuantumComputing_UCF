...
2)By definition of conjugate transpose, and assuming A has real entries, the conjugate \indent\indent transpose of A reduces to the transpose of A.
\newline
\newline
...
\indent4) Keeping the same expression on the LHS, now we are at \(\< A | B\>_{\mathrm{Tr}} = \mathrm{Tr}(AB).\newline
\newline
 \indent\indent\(\< A | B\>_{\mathrm{Tr}} $  is also by definition the inner product trace. This Definition states that for \indent\indent real square matrices of the same size, \< A | B\>  = {\mathrm{Tr}}\((A B^T).\newline
 

\indent5)From 1, \)\(Tr(B)  = Tr(B^T)\), thus \< A | B\>  \Rightarrow \mathrm{Tr}(AB).\newline
